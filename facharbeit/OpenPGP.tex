...
Speichert man einige weitere Zahlen, erh"alt man auch zwei ganze Zahlen $x, y$, sodass
$xa + yb = ggt(a, b)$~\cite{taocp2}.

Um $d$ zu bestimmen, wenn $ed \equiv 1 \mod \phi(n) $ und $e$ sowie $\phi(n)$ bekannt sind,
kann der erweiterte Euklidsche Algorithmus wegen der folgenden Verh"altnisse verwendet werden,
wobei $x, y \in\mathbb{Z}$ (mittels des Algorithmus) so sind, dass $xe + y\phi(n) = 1$
und $ed \equiv 1 \mod \phi(n)$ als $ed = k \phi(n) + 1$ f"ur ein $k \in \mathbb{Z}$:
\[\begin{aligned}
      &ed &=&~k \phi(n) + 1 \\
      &&=&~k\phi(n) + xe + y\phi(n) \\
      &&=&~xe + (k+y)\phi(n) \\
      \iff& ed &\equiv&~xe \mod \phi(n) \\
