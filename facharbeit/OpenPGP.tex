...
$\phi(pq) = \phi(p) \cdot \phi(q) = (p-1)(q-1)$~\cite{euler63}. \todo{Name f"ur die Gleichung?}
...
$sa_0 + tb_0 = \textrm{ggt}(a, b)$~\cite{taocp2}. \todo{Name f"ur die Gleichung?}
...
\begin{enumerate}
    ...
    \item Falls $b = 0$: Beende, es gilt: $\textrm{ggt}(a_b, b_0) = a = s_1 a_0 + t_1 b_0$
...
erh"alt man f"ur die linke Seite der Kongruenz~\cite{rsa}: \todo{vertikal?}
\[
...
und die Faktorisierung von $n$ "aquivalent sind:
...
f"ur die das Bit an der STelle $i$ in der binaren Darstellung von $b$ gleich $1$ ist.
