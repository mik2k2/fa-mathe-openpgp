...
Die Faktorisierung wird jedoch schon seit Jartausenden untersucht,
...
jedoch kann diese Kenntnis wie folgt in Kenntnis von $p$ und $q$
umgewandelt werden, sodass die Berechnung von $\phi(n)$
und die Faktorisierung $n$s "aquivalent sind.
...
der die Differenz $p-q$ und folgend $q = \frac{(p+q) - (p-q)}{2}$ berechnet werden.
